...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $phone_number = isset($_POST['phone_number']) ? $conn->real_escape_string($_POST['phone_number']) : NULL; // phone_number is nullable
    ...
    if ($first_name && $last_name && $email && $username && $password && $confirm_password) {
        ...
        $sql = "INSERT INTO admin (first_name, last_name, email, phone_number, username, password_hash, role, status, created_at) 
                VALUES ('$first_name', '$last_name', '$email', '$phone_number', '$username', '$hashed_password', '$role', '$status', CURRENT_TIMESTAMP())";
